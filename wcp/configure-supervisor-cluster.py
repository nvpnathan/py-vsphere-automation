...
inputs = {'host': config['VC_IP'],
          'password': config['VC_SSO_PWD'],
          'user': 'administrator@vsphere.local',
          'clustername': config['VC_CLUSTER'],
          'wcpsize': config['WCP_SIZE'],
          'dvs_name': config['VDS_NAME'],
          'mastervmnetwork': config['WCP_MASTERVMNET'],
          'startingip': config['WCP_MASTERSTARTINGIP'],
          'mastersm': config['WCP_MASTERMASK'],
          'gatewayip': config['WCP_MASTERGATEWAY'],
          'dnsserver': config['VC_DNS_SERVERS'],
          'ntpserver': config['NTP_SERVER'],
          'storagepolicy': config['WCP_STORAGEPOLICY'],
          'dnsserver': config['VC_DNS_SERVERS'],
...
dvs_object = s.get(
    'https://' + args.host + '/api/vcenter/namespace-management/distributed-switch-compatibility?cluster=' + cluster_id + '&compatible=true')
...
edge_object = s.get(
    'https://' + args.host + '/api/vcenter/namespace-management/edge-cluster-compatibility?cluster=' + cluster_id + '&compatible=true&distributed_switch=' + dvs_id)
...
json_response = s.post(
    'https://' + args.host + '/api/vcenter/namespace-management/clusters/' + cluster_id + '?action=enable',
